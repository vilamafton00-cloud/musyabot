...
class Levels(commands.Cog):
    ...
    async def on_message(self, message):
        # НЕ БЛОКИРУЕМ КОМАНДЫ — просто обрабатываем EXP
        if message.author.bot or not message.guild:
            return

        # Не даём EXP за команды
        ...
        for guild in self.bot.guilds:
            for vc in guild.voice_channels:
                ...
                for member in members:
                    exp = config.EXP_PER_VOICE_MINUTE * 5
    ...
    async def rank(self, ctx, member: discord.Member = None):
        ...
        embed.add_field(name="📝 Сообщений", value=f"```{user['total_messages']:,}```", inline=True)
        embed.add_field(name="🎙️ В войсе (мин)", value=f"```{user['total_voice_minutes']:,}```", inline=True)
    ...
    async def leaderboard(self, ctx, page: int = 1):
        ...
        if my_pos:
            embed.set_footer(
                text=f"Ваша позиция: #{my_pos} • Страница {page}/{total_pages} • Musya-Bot",
    ...
    async def rewards(self, ctx):
        ...
        for level, role_id in sorted(config.LEVEL_ROLES.items()):
            ...
            embed.add_field(name=f"Уровень {level}", value=role_name, inline=True)
